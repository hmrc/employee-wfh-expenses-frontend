...
class DateFormatterSpec extends SpecBase with Matchers {
  ...
  "testFormatter.bind" when {
    ...
    "the formatter has non inclusive date range" must {
      "return global date error when date is valid but is too far in the future" in {
        ...
        ) mustBe Left(List(FormError(testKey, Errors.dateMaxError, List(dateFormatter.format(testMaxDate)))))
      }
      "return global date error when date is valid but is too far in the past" in {
        ...
        ) mustBe Left(List(FormError(testKey, Errors.dateMinError, List(dateFormatter.format(testMinDate)))))
      }
