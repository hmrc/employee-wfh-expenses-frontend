...
class SelectTaxYearsToClaimForSpec extends WordSpec with MustMatchers with ScalaCheckPropertyChecks with OptionValues with ModelGenerators {
    ...
    "serialise" in {
      ...
      forAll(gen) {
        selectTaxYearsToClaimFor =>
          ...
          Json.toJson(selectTaxYearsToClaimFor) mustEqual JsString(selectTaxYearsToClaimFor.toString)
      }
    }
  }
}
