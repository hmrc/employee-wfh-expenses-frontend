...
class IABDServiceImpl @Inject()(
  ...
  def getAlreadyClaimedStatusForAllYears(nino: String)(implicit hc: HeaderCarrier): Future[(Option[Expenses], Option[Expenses], Option[Expenses], Option[Expenses])] = {
    val alreadyClaimed2020Future = alreadyClaimed(nino, YEAR_2020)
    val alreadyClaimed2021Future = alreadyClaimed(nino, YEAR_2021)
    val alreadyClaimed2022Future = alreadyClaimed(nino, YEAR_2022)
    val alreadyClaimed2023Future = alreadyClaimed(nino, YEAR_2023)
    ...
    for {
      ...
      (alreadyClaimed2020, alreadyClaimed2021, alreadyClaimed2022, alreadyClaimed2023)
    }
  }

  def allYearsClaimed(nino: String, yearsClaimed: (Option[Expenses], Option[Expenses], Option[Expenses], Option[Expenses]), audit: Boolean = true)(implicit hc: HeaderCarrier): Boolean = {
    yearsClaimed match {
      case (Some(claimed2020), Some(claimed2021), Some(claimed2022), Some(claimed2023)) =>
        logger.info(s"[IABDService][hasClaimedForAllYears] Detected already claimed for $YEAR_2020, $YEAR_2021, $YEAR_2022 and $YEAR_2023; redirecting to P87 digital form")
        if(audit) {
