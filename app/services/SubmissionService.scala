...
(
  ...
  def submitExpenses(startDate: Option[LocalDate])
  ...
  private def submit(startDate: Option[LocalDate])
    ...
    val flatRateItems: Seq[FlatRateItem] = startDate match {
      case Some(date) => (dataRequest.userAnswers.is2019And2020And2021Only, dataRequest.userAnswers.is2019And2020Only) match {
        case (true, false) =>
          if (date.isAfter(TAX_YEAR_2019_END_DATE)) { // Claiming for 2020 and 2021
            Seq[FlatRateItem](
              ...
              FlatRateItem(year = YEAR_2021, amount = calculate2021FlatRate(date.isAfter(TAX_YEAR_2020_END_DATE)))
            )
      ...
      case None => // Claiming for 2021
        Seq[FlatRateItem](
          FlatRateItem(year = YEAR_2021, amount = calculate2021FlatRate(true))
        )
    }
  ...
  def calculate2021FlatRate(is2021BeingClaimed: Boolean): Int = {
    if(is2021BeingClaimed) {
